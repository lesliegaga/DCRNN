...
class GCGRUCell(RNNCell):
    ...
    def __init__(self, num_units, adj_mx, max_diffusion_step, num_nodes, num_proj=None,
        ...
        """
        super(GCGRUCell, self).__init__(_reuse=reuse)
        ...
        elif filter_type == "dual_first_order":
            supports.append(utils.calculate_first_order_matrix(adj_mx))
            supports.append(utils.calculate_first_order_matrix(adj_mx.T))
        else:
    ...
    def _build_sparse_matrix(L):
        L = L.tocoo()
        indices = np.column_stack((L.row, L.col))  # row和col保存的是行列索引值
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        """
        with tf.variable_scope(scope or "gcgru_cell"):
    ...
    def _gconv(self, inputs, state, output_size, bias_start=0.0):
        ...
        with tf.variable_scope(scope):
            ...
            num_matrices = len(self._supports)
